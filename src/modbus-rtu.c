 ...
 *
 * Modifications:
 * - Custom serial read, write, and flush mechanisms
 * - Hardware abstraction for different MCU platforms
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#else
	modbus_rtu_t *ctx_rtu = ctx->backend_data;
  	pstUartDevice l_pstUartDevice = (pstUartDevice)ctx_rtu->low_level_dev;
    unsigned int  l_u32Timeout = (unsigned int) ((ctx->response_timeout.tv_sec * 1000000 + ctx->response_timeout.tv_usec)/1000);
  	return l_pstUartDevice->m_pFnSend(l_pstUartDevice, (uint8_t *)req, req_length,l_u32Timeout);
#endif
}
...
static ssize_t _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
  ...
  unsigned int  l_u32Timeout = (unsigned int) ((ctx->response_timeout.tv_sec * 1000000 + ctx->response_timeout.tv_usec)/1000);
  if (l_u32Timeout == 0)
      l_u32Timeout = MODBUS_MAX_DELAY;
  return l_pstUartDevice->m_pFnRecv(l_pstUartDevice, rsp, rsp_length, l_u32Timeout);
#endif
}
    ...
    if (req[0] != rsp[0] && req[0] != MODBUS_BROADCAST_ADDRESS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    req[0]);
#else
            modbus_printf("The responding slave %d isn't the requested slave %d\n",
                    rsp[0],
                    req[0]);
#endif
        }
    ...
    if (crc_calculated != crc_received) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    crc_calculated);
#else
            modbus_printf("ERROR CRC received 0x%0X != CRC calculated 0x%0X\n",
                    crc_received,
                    crc_calculated);
#endif
        }
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
  ...
  /* to avoid reinitializing the uart peripheral */
  if ((l_pstUartDevice->m_pvSendSem == NULL) || (l_pstUartDevice->m_pvRecvSem == NULL)) {
    l_pstUartDevice->m_pFnInit(l_pstUartDevice, ctx_rtu->baud, ctx_rtu->parity, ctx_rtu->data_bit, ctx_rtu->stop_bit);
  }
  ctx->s = 1;
  return 0;
}
...
static int _modbus_rtu_flush(modbus_t *ctx)
{
  ...
  modbus_rtu_t *ctx_rtu = ctx->backend_data;
  pstUartDevice l_pstUartDevice = (pstUartDevice)ctx_rtu->low_level_dev;
  return l_pstUartDevice->m_pFnFlush(l_pstUartDevice);
#endif
}
...
s_rc=1;
if (ctx->debug) {
                modbus_printf("Modbus RTU for MCU does not support the select function\n");
            }
#endif
    return s_rc;
}
...
#else
        modbus_printf("The device string is empty\n");
#endif
        errno = EINVAL;
        return NULL;
    }
...
#else
        modbus_printf("The baud rate value must not be zero\n");
#endif
        errno = EINVAL;
        return NULL;
    }
...
#ifdef __MCU__	
    ctx_rtu->low_level_dev = find_uart_device(ctx_rtu->device);
    if (ctx_rtu->low_level_dev == NULL)
    {
      modbus_free(ctx);
      errno = ENOMEM;
      return NULL;
