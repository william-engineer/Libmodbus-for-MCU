 ...
 * 
 * Modifications Copyright © William Chen <william_engineer@outlook.com>
 * SPDX-License-Identifier: LGPL-2.1-or-later
 *
 * Modifications:
 * - Replacement of socket-based functions with MCU-specific interfaces
 */
...
#else
    return modbus_net_send(ctx->s, (const char *) req, req_length, MSG_NOSIGNAL);
#endif
}
...
#else
    return modbus_net_recv(ctx->s, (char *) rsp, rsp_length, 0);
#endif
}
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    (req[0] << 8) + req[1]);
#else
            modbus_printf("Invalid transaction ID received 0x%X (not 0x%X)\n",
                    (rsp[0] << 8) + rsp[1],
                    (req[0] << 8) + req[1]);
#endif
        }
...
#else
            modbus_printf("Invalid protocol ID received 0x%X (not 0x0)\n", protocol_id);
#endif
        }
        errno = EMBBADDATA;
        return -1;
...
#else
    rc = modbus_net_setsockopt(s, IPPROTO_TCP, TCP_NODELAY, &option, sizeof(int));
#endif
    if (rc == -1) {
        return -1;
    }
...
#else
    modbus_net_ioctl(s, FIONBIO, &option);
#endif
...
#else
    rc = modbus_net_setsockopt(s, IPPROTO_IP, IP_TOS, &option, sizeof(int));
#endif
    if (rc == -1) {
        return -1;
    }
...
#else
    int rc = modbus_net_connect(sockfd, addr, addrlen);
#endif
...
#else
        rc = modbus_net_select(sockfd + 1, NULL, &wset, NULL, &tv);
#endif
        if (rc < 0) {
            /* Fail */
            return -1;
...
#else
        rc = modbus_net_getsockopt(sockfd, SOL_SOCKET, SO_ERROR, (void *) &optval, &optlen);
#endif
        if (rc == 0 && optval == 0) {
            return 0;
        } else {
...
static int _modbus_tcp_connect(modbus_t *ctx)
{
...
#else
    ctx->s = modbus_net_socket(PF_INET, flags, 0);
#endif
    if (ctx->s < 0) {
        return -1;
    }
    ...
    if (rc == -1) {
        ...
        close(ctx->s);
#else
        modbus_net_shutdown(ctx->s, SHUT_RDWR);
        modbus_net_close(ctx->s);
#endif
        ctx->s = -1;
        return -1;
...
#else
        rc = modbus_net_recv(ctx->s, devnull, MODBUS_TCP_MAX_ADU_LENGTH, MSG_DONTWAIT);
#endif
#else
...
#else
    new_s = modbus_net_socket(PF_INET, flags, IPPROTO_TCP);
#endif
    if (new_s == -1) {
        return -1;
    }
...
#else
    enable = 1;
    if (modbus_net_setsockopt(new_s, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(enable)) == -1) {
        modbus_net_close(new_s);
        return -1;
    }
#endif
...
#else
        rc = modbus_net_inet_pton(addr.sin_family, ctx_tcp->ip, &(addr.sin_addr));
        if (rc <= 0) {
            if (ctx->debug) {
                modbus_printf("Invalid IP address: %s\n", ctx_tcp->ip);
            }
            modbus_net_close(new_s);
            return -1;
        }
...
#else
    if (modbus_net_bind(new_s, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
        modbus_net_close(new_s);
        return -1;
    }

    if (modbus_net_listen(new_s, nb_connection) == -1) {
        modbus_net_close(new_s);
        return -1;
    }
#endif
...
#else
    ctx->s = modbus_net_accept(*s, (struct sockaddr *) &addr, &addrlen);
#endif
#endif
...
#else
        if (modbus_net_inet_ntop(AF_INET, &(addr.sin_addr), buf, INET_ADDRSTRLEN) == NULL) {
            modbus_printf("Client connection accepted from unparsable IP.\n");
#endif
		} else {
#ifndef __MCU__
            printf("Client connection accepted from %s.\n", buf);
#else
		    modbus_printf("Client connection accepted from %s.\n", buf);
#endif
        }
    }
...
#else
    while ((s_rc = modbus_net_select(ctx->s + 1, rset, NULL, NULL, tv)) == -1) {
#endif
        if (errno == EINTR) {
            if (ctx->debug) {
#ifndef  __MCU__
...
#else
                modbus_printf("A non blocked signal was caught\n");
#endif
            }
...
#else
            modbus_printf("The IP string is empty\n");
#endif
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
...
#else
            modbus_printf("The IP string has been truncated\n");
#endif
            modbus_free(ctx);
            errno = EINVAL;
            return NULL;
