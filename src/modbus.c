...
#else
    if (ctx->debug) {
        modbus_printf("ERROR %s", modbus_strerror(errno));
        if (context != NULL) {
            modbus_printf(": %s\n", context);
        } else {
            modbus_printf("\n");
...
#else
	 os_delay_ms((ctx->response_timeout.tv_sec * 1000) + (ctx->response_timeout.tv_usec / 1000));
#endif
#endif
}
...
#else
            modbus_printf("ERROR The connection is not established.\n");
#endif
        }
        return -1;
    }
...
#else
        if ((ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_TCP) && (rc == 0)) {
            errno = ECONNRESET;
            rc = -1;
        }
#endif
        ...
        if (function != req[offset]) {
            if (ctx->debug) {
                ...
                fprintf(
                    ...
                    req[offset]);
#else
                modbus_printf("Received function not corresponding to the request (0x%X != 0x%X)\n",
                    function,
                    req[offset]);
#endif
            }
        ...
        if ((req_nb_value == rsp_nb_value) && (resp_addr_ok == TRUE) &&
            ...
            if (ctx->debug) {
                ...
                fprintf(stderr,
                        ...
                        req_nb_value);
#else
                modbus_printf("Received data not corresponding to the request (%d != %d)\n",
                        rsp_nb_value,
                        req_nb_value);
#endif
            }
...
#else
            modbus_printf("FIXME Not implemented\n");
#endif
        }
        errno = ENOPROTOOPT;
        return -1;
    ...
    if (nb > MODBUS_MAX_READ_BITS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_READ_BITS);
#else
            modbus_printf("ERROR Too many bits requested (%d > %d)\n",
                    nb,
                    MODBUS_MAX_READ_BITS);
#endif
        }
    ...
    if (nb > MODBUS_MAX_READ_BITS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_READ_BITS);
#else
            modbus_printf("ERROR Too many discrete inputs requested (%d > %d)\n",
                    nb,
                    MODBUS_MAX_READ_BITS);
#endif
        }
    ...
    if (nb > MODBUS_MAX_READ_REGISTERS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_READ_REGISTERS);
#else
            modbus_printf("ERROR Too many registers requested (%d > %d)\n",
                    nb,
                    MODBUS_MAX_READ_REGISTERS);
#endif
        }
    ...
    if (nb > MODBUS_MAX_READ_REGISTERS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_READ_REGISTERS);
#else
            modbus_printf("ERROR Too many input registers requested (%d > %d)\n",
                    nb,
                    MODBUS_MAX_READ_REGISTERS);
#endif
        }
    ...
    if (nb > MODBUS_MAX_WRITE_BITS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_WRITE_BITS);
#else
            modbus_printf("ERROR Writing too many bits (%d > %d)\n",
                    nb,
                    MODBUS_MAX_WRITE_BITS);
#endif
        }
    ...
    if (nb > MODBUS_MAX_WRITE_REGISTERS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_WRITE_REGISTERS);
#else
            modbus_printf("ERROR Trying to write to too many registers (%d > %d)\n",
                    nb,
                    MODBUS_MAX_WRITE_REGISTERS);
#endif
        }
    ...
    if (write_nb > MODBUS_MAX_WR_WRITE_REGISTERS) {
        if (ctx->debug) {
            ...
            fprintf(stderr,
                    ...
                    MODBUS_MAX_WR_WRITE_REGISTERS);
#else
            modbus_printf("ERROR Too many registers to write (%d > %d)\n",
                    write_nb,
                    MODBUS_MAX_WR_WRITE_REGISTERS);
#endif
        }
